...
class FavoriteFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        observeFavoriteEvents(eventAdapter)
    }

    private fun observeFavoriteEvents(adapter: EventAdapter) {
        viewModel.favoriteEvents.observe(viewLifecycleOwner) { data ->
            val isEmpty = data.isNullOrEmpty()
            showStatusMessage(isEmpty, if (isEmpty) getString(R.string.status_no_favorites_found) else null)
